...
export function ProjectPage() {
  ...
  React.useEffect(() => {
    ...
    else if (path.includes('/assets')) setActiveTab('assets');
    ...
    <PageLayout projectId={projectId}>
      ...
      <PageContent>
        ...
        {activeTab === 'assets' && (
          <div className="space-y-6">
            <div>
              <h2 className="font-heading text-xl text-foreground mb-2">Asset Library</h2>
              <p className="text-sm text-muted-foreground">
                Upload and manage images, audio, and video for your atmospheric states
              </p>
            </div>

            <AssetUploader
              projectId={projectId}
              onUploadComplete={() => setAssetsRefreshKey(prev => prev + 1)}
            />
